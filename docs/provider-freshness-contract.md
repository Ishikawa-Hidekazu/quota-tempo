...
- A valid observation from either local source succeeds independently; a malformed or oversized optional sibling source does not turn it into a failed refresh. An unsafe selected path remains a failed refresh even if the sibling source succeeds. Otherwise, report a local read failure only when neither source yields a valid observation.
