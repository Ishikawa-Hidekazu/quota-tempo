...
2. It decodes only `cachedUsageUtilization` from Claude Code's local configuration with an 8 MiB ceiling. When Desktop history is newer, QuotaTempo combines its utilization with cached reset timestamps only if both observations fall within the same five-hour or weekly window. A complete current local result supplies utilization and reset timestamps without launching a process.
   A valid observation from either local source remains usable when the optional sibling source is malformed or oversized. A symlink-selected or otherwise unsafe path still fails the refresh before any live probe, even when the sibling source is valid.
3. When those local observations do not contain current exact reset windows, the app resolves and verifies the installed official Claude Code executable for every refresh, starts it in a bounded PTY, and reads only the rendered current-session and all-model weekly rows from `/usage`. The older noninteractive `get_usage` decoder remains test-only because that response does not contain the required quota windows.
...
The selected adapter never opens browser cookie databases, reads Keychain or OAuth values, refreshes provider tokens, calls the Claude web usage endpoint, or records prompts or transcripts. The PTY uses the provider-owned CLI's existing sign-in, removes its generated session artifact after each attempt and at orderly app shutdown, and stores only normalized percentages, duration, reset when present, its estimate marker, capture time, source, and acquisition state.
...
Tests use fake Codex JSON-RPC results, fake Claude control responses, temporary Claude Desktop history and cache fixtures, real bounded local shell subprocesses, synthetic status-line input, and temporary settings directories. They cover local-first selection, first-run provider detection, single-provider filtering, the minimum-one-provider rule, live CLI re-resolution, monotonic observation selection, compatible-window merge, one-window reset projection, refusal to chain estimates, per-window invalid and expired reset isolation, incompatible reset rejection, resolved and signature-verified executables, bounded PTY parsing, environment overrides, early child exit, process-group termination, serialized ordered output at process exit, refresh timing, the 15-minute schedule, clock-driven freshness changes, timeout, input and output ceilings, partial input reads, exact status-line payload forwarding, malformed percentages and resets, Codex usage restrictions, reset-unknown display, five-hour risk without a weekly window, displayed rounding consistency, never-observed defaults, source-only field exclusion, failed refresh preservation, visible atomic write failure as the next refresh input, duplicate-provider display, status-line coexistence, ownership drift, rollback, uninstall, selected-path symlink rejection, and resource lookup when SwiftPM is invoked outside the repository root. The app-bundle test exercises every trigger with providers disabled, validates distributable file modes and bundled policy documents, and requires both Codex and Claude records to remain absent.

## Automatic Claude metadata-only live check

Repeated bounded PTY checks against the installed official Claude Code CLI returned `panelCaptured` with exact five-hour and weekly reset windows in both the normal probe directory and a fresh owner-only directory. The noninteractive `get_usage` compatibility check remains disabled because its response does not expose those windows. The checks did not emit or retain quota values, reset values, organization identifiers, raw source JSON, credentials, cookies, tokens, prompts, or transcripts in diagnostics; temporary probe directories and generated session artifacts were removed.
