...
QuotaTempoは起動時、起動中の15分ごと、Macのスリープ復帰時に、有効なproviderだけを範囲限定で取得します。最後の試行から5分以上経過している場合は、メニューを開いたときにも更新することがあります。自動更新とメニュー表示時の更新は、同じproviderの取得中に二重実行しません。**更新**を押すと、有効なproviderをすぐに再取得します。取得中は**更新中…**と表示し、buttonを無効化します。
