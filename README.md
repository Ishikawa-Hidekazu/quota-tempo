...
- The next build adds an interactive `/usage` fallback. It uses a complete, current local cache when available; otherwise it strictly parses the current CLI panel. The app schedules automatic refresh every 15 minutes and uses a 14-minute last-attempt guard so timer jitter cannot skip a cycle; explicit Refresh probes immediately. An ambiguous or failed probe keeps an earlier exact current observation and capture time regardless of whether it came from the CLI or local cache. Claude Code's nonessential-traffic suppression is intentionally not enabled because it also blocks the usage request. The old noninteractive `get_usage` decoder remains test-only.
